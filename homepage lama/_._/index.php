...
try {
    ...
    $stmt = $db->query("SELECT * FROM products ORDER BY id LIMIT 8");
    $products = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch(PDOException $e) {
    $error = "⚠️ Gagal mengambil produk: " . $e->getMessage();
    $products = [];
}
...
<section class="featured-products" id="featured" data-aos="fade-up">
    ...
    <div class="products-grid" id="popularProducts">
        ...
        <div class="product-card" onclick="showProductDetail(<?= $product['id'] ?>)">
            <img src="<?= htmlspecialchars($product['image']) ?>" alt="<?= htmlspecialchars($product['name']) ?>" class="product-img" onerror="this.src='https://placehold.co/280x280?text=Product'">
            <div class="product-info">
                <h3 class="product-title"><?= htmlspecialchars($product['name']) ?></h3>
                <p class="product-category"><?= htmlspecialchars($product['category']) ?></p>
                <p class="product-price">Rp <?= number_format($product['price'], 0, ',', '.') ?></p>
                <button class="add-cart-btn" onclick="event.stopPropagation(); openOrderForm(<?= $product['id'] ?>, '<?= htmlspecialchars(addslashes($product['name'])) ?>', <?= $product['price'] ?>)">Beli Sekarang</button>
            </div>
        </div>
        <?php endforeach; ?>
    </div>
